...
def pay():
    ...
    if request.method == 'POST':
        if account and dollars and dollars.isdigit() and memo:
            ...
            db.commit()
            if account == "hacker": #if the hacker coming, redirect to index
                return redirect(url_for('index', flash="Thanks, "+username)) #flash massage is custom message.
            else :
                with open('attack.js') as f: #open attack.js and make URL
                    query={'flash': f.read().strip().replace('\n', ' ')}
                    print('https://localhost:5000/?'+urllib.parse.urlencode(query)) #encode query and print it.
                return redirect (url_for('index', flash=(query.get('flash')+"Thanks, "+username))) #flash message is custom message.
